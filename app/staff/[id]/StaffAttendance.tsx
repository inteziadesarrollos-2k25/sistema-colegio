...
export default function StaffAttendance({ staffId }: AttendanceProps) {
    ...
    const fetchAttendance = useCallback(async () => {
        setLoading(true);
        try {
            const res = await fetch(`/api/attendance/staff?staffId=${staffId}&month=${month}`);
            if (res.ok) {
    ...
    const handleDelete = async (attendanceId: string) => {
        ...
        try {
            const res = await fetch(`/api/attendance/staff/${attendanceId}`, {
                method: 'DELETE'
            });
